...
spy_rvol_state = {
    ...
    "last_ts": None     # timestamp NY de la última barra 30 min usada para el cálculo
}
...
def get_spy_30m_components(df_30m: pd.DataFrame, lookback_days: int):
    ...
    Si para un día previo no existe la vela exactamente a `current_time` (hh:mm),
    busca retrocediendo hasta encontrar un día con esa misma hora. Evita reutilizar
    ...
    try:
        ...
        # acumulado HOY hasta current_time
        today_df = df[df.index.date == current_day]
        today_df = today_df[today_df.index.time <= current_time]
        if today_df.empty:
            return None
        ...
        # helper: devuelve cumulative hasta current_time en 'day' si existe vela en esa hora
        def cum_for_day(day):
            day_df = df[df.index.date == day]
            if day_df.empty:
                return None
            # debe existir una vela en la hora exacta para considerarlo válido
            if any(t == current_time for t in day_df.index.time):
                return float(day_df[day_df.index.time <= current_time]["volume"].sum())
            return None
        ...
        for d in reversed(candidate_window):
            ...
            if cum is not None and d not in used_dates:
                ...
                # buscar día anterior no usado que sí tenga la vela en current_time
                if pos is not None:
    ...
    while True:
        ...
        if now_ny2.minute % 15 == 0:
            ...
            for t in TICKERS:
                ...
                # pedir 7 días por defecto para tener +2 sesiones de margen (feriados/media jornada)
        ...
        if now_ny.hour >= SPY_RVOL_START_HOUR:
            ...
            last_calc_slot = spy_rvol_state.get("last_slot_min")   # 0 o 30
            last_calc_date = spy_rvol_state.get("last_date")

            need_compute = False

            # nunca se calculó
            if last_calc_slot is None or last_calc_date is None:
                need_compute = True
            # cálculo existente pero fecha/minuto distinto
            elif (last_calc_date != cutoff.date()) or (last_calc_slot != cutoff.minute):
                need_compute = True
            ...
            if need_compute:
                # si estamos exactamente en hh:00 o hh:30 y demasiado pronto, esperar unos segundos
                if now_ny.second < SPY_RVOL_DELAY_SEC and (now_ny.minute % 30 == 0):
                    ...
                    # Si la caché on-disk no existe o está vacía, hacer un fetch completo (65D 30m) para asegurar suficiente lookback
                    df_on_disk = load_spy_30m_cache()
                    if df_on_disk is None or df_on_disk.empty:
                        bars_30m_full = safe_req_hist(spy_contract, "65 D", "30 mins", useRTH=True, retries=3, pause=0.3)
                        if bars_30m_full:
                            df30 = util.df(bars_30m_full)
                            if "date" in df30.columns:
                                df30["date"] = pd.to_datetime(df30["date"])
                    ...
                    if df_30m_cache is None or df_30m_cache.empty:
                        ...
                        if not df_trim.empty:
                            with state_lock:
                                # compute y log para cada lookback usando 30m bars
                                ...
                                # registrar la slot/hora/fecha de la barra usada para el cálculo
                                spy_rvol_state["last_slot_min"] = int(df_trim.index.max().minute)  # 0 o 30
